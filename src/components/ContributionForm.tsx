...
const ContributionForm: React.FC<ContributionFormProps> = ({ campaignId, campaign, onSuccess }) => {
  ...
  const { account: activeAccount, connectWallet, isConnecting, chainId, switchToShannonTest } = useWallet();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (chainId !== 50312) { // Sepolia chain ID
      setError('Please switch to Somnia Shannon');
      return;
    }
  ...
  if (chainId && chainId !== 50312) { 
    return (
      <div className="bg-white rounded-lg p-6 shadow-md">
        ...
        <div className="text-center py-8">
          <p className="text-gray-600 mb-4">Please switch to Somnia Shannon to contribute</p>
          <button
            onClick={switchToShannonTest}
            ...
            <div className="mb-4">
              <div className="p-4 rounded-md bg-green-50 border border-green-200">
                ...
                <div className="text-xs text-green-600">
                  <p>Total raised: {campaign.currentAmount.toFixed(3)} STT</p>
                  <p>Available to withdraw: {(campaign.currentAmount * 0.95).toFixed(3)} STT (after 5% platform fee)</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="relative rounded-md shadow-sm">
                ...
                <div className="absolute inset-y-0 left-15 pl-15 flex items-center pointer-events-none">
                  <span className="text-gray-500 sm:text-sm">STT</span>
                </div>
              </div>
              ...
              <div className="p-3 rounded-md" style={{backgroundColor: 'rgba(17, 25, 38, 0.05)'}}>
                ...
                {amount && !isNaN(parseFloat(amount)) && parseFloat(amount) > 0 && (
                  <div className="text-xs" style={{color: '#374151'}}>
                    <p>Your contribution: {parseFloat(amount).toFixed(3)} STT</p>
                    <p>Platform fee (5%): {(parseFloat(amount) * 0.05).toFixed(3)} STT</p>
                    <p className="font-medium">Campaign receives: {(parseFloat(amount) * 0.95).toFixed(3)} STT</p>
                  </div>
                )}
              </div>
            </div>
